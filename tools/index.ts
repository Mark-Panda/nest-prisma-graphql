...
async function autoGenerated() {
    ...
    console.log('configYml', configYml.rolePermission);
    ...
    for (const modelItem of schema.list) {
        if (modelItem.type === 'model') {
            ...
            const allGenerateFunc = [
                ...
                lowerModel + 'GroupBy',
                'createMany' + modelName + 's',
                'updateMany' + modelName + 's',
                'deleteMany' + modelName + 's',
            ];
            const permissiongroup = new Map();
            for (const itemFunc of allGenerateFunc) {
                let roleString = '';
                const adminRole: any = configYml.rolePermission.admin.filter(
            ...
            writeFileSync(
                ...
                export class ${modelName}Resolver {
                    ...
                    @Query(() => [${modelName}])
                    @UseGuards(GqlAuthGuard)
                    ${permissiongroup.get(lowerModel + 's')}
                    async ${lowerModel}s(
                    ...
                    @UseGuards(GqlAuthGuard)
                    ${permissiongroup.get('createMany' + modelName + 's')}
                    ...
                    @Mutation(() => AffectedRows, { nullable: true })
                    @UseGuards(GqlAuthGuard)
                    ${permissiongroup.get('updateMany' + modelName + 's')}
                    ...
                    @Mutation(() => AffectedRows, { nullable: true })
                    @UseGuards(GqlAuthGuard)
                    ${permissiongroup.get('deleteMany' + modelName + 's')}
